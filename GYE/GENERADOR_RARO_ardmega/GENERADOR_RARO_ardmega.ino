...
String modo = "A";              // modo automatico por defecto
...
//  periodo_ext = (1/frecuencia_ext)*1000;    
...
void analizar_modo(void) 
{
  ...
  {
    temp = 0x01;
    cadena =  (String(RPM_int) + ',' + String(frecuencia_int) + ',' + String(valorx) + ',' + String(stroken) + ',' + String(freq1));
  ...
  {
    temp = 0x00;
    cadena =  (String(RPM_ext) + ',' + String(frecuencia_ext) + ',' + String(valorx) + ',' + String(stroken) + ',' + String(freq1));
    digitalWrite(13, (bool) temp);    // imagen de la senal temp
  }
}
...
void descomponer_trama(void)
{
  ...
  /*Tipo de actuador*/ 
    ...
    /* measure */   
    temp_str = buffer_uart.substring(18,19);
    temp_int = temp_str.toInt();
...
void lanzar_stroken()
{
  ...
  if (cont2 > stroken)              // una vez alcanzado stroken se apagan las EV
  {
    ...
    cont2 = stroken + 1;            // DEJAR AL CONTADOR FUERA DEL RANGO STROKEN para volver a apagar las EV - es decir volver a esta condicion o a la siguiente
...
ISR (TIMER0_COMPA_vect)
{
  ...
  if ((temp == 0) && (cont >= (dutty1 + peak + 6)))       // si modo automatio y contador acabo el ciclo  PARA AGREGAR DESFASE EN A SE DEBERIA SUMAR EL DESFASE EN A A LA CONDICION 2
      cont = dutty1 + peak + 6 + 20000;                   // Dejar el contador fuera del rango pero que no vuelva a cero para hacerlo con la INT0 = PARA DESFASE A SE DEBE SUMAR EL DESFASE MAXIMO AL 300 
                                                          // 20.000 es el numero maximo de frecuencia no puede tomar un valor mayor a ese     
...
/*========= Canal B ========*/
  ...
  {    
    ...
    if (cont_b < freq1)
     ...
     if (aux_led > 1000 )             // periodo 1000 es de 50ms - se puede cambiar este limite si se desea tooglear el led mas rapido o mas lento 
    {
      PORTC ^= 0B10000000;            // TOGLEAMOS LED INDICADOR DE CICLO B EN FUNCIONAMIENTO
      aux_led = 0;
    }
  }
  if ((temp == 0) && (cont_b >= (dutty2 + peak + 6 + desfase)))      // aqui toca aumentar mas desfase para no borrar el canal b analizar mejor ya que desfase si puede ser mayor a 300 creo que tocaria ponerle en algo mayior a 20.000 o tener contador A contrador B 
      cont_b = dutty2 + peak + 6 + desfase + 20000;                  // Dejar el contador fuera del rango pero que no vuelva a cero para hacerlo con la INT0  - 20000 es el numero maximo de frecuencia no puede tomar un valor mayor a ese     
...
ISR (INT0_vect)
{
  ...
  if ((temp == 0) && (cont_b >= (dutty2 + peak + 6 + desfase)))
    cont_b = 0;
}
